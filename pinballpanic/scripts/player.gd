	...
	#@export var max_fall_speed := 1600
	#
...
extends CharacterBody2D
#movement
@export var speed: float = 450.0          #speed max buat lari
@export var acceleration: float = 2500.0  #akselerasi waktu lari (makin tinggi makin ngebut)
@export var friction: float = 3000.0      #parameter ngerem (lebih besar dari akselerasi biar ngeremnya pakem)

#jump
@export var jump_force: float = -750.0    # Kekuatan dorong ke atas
@export var gravity: float = 1400.0       #gravitasi dasar
@export var fall_gravity_multiplier: float = 1.5 #akselerasi waktu jatuh
...
#func handle_movement(delta):
	...
	##coyottetime
		...
		#coyote_timer -= delta        
			...
			#if velocity.y > max_fall_speed:
				#velocity.y = max_fall_speed
		#elif abs(velocity.y) < 200: #hangtime
